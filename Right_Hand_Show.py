...
# Importing necessary packages for Right Palm Mediapipe
...
class RightShow:
    ...
    def __init__(self, videopath):
        ...
        # Setting up the initial parameters for right hand detection
        ...
        Path('videoliveliness_files_right_hand').mkdir(exist_ok=True, parents=True)
        self.PATH = "videoliveliness_files_right_hand/"
        ...
        # Initial Right_hand_count status
        self.Right_count = {'status_flag': self.STATUS_FLAG, 'Right_hand_count': self.COUNT_SHOW}

    def liveliness_check(self):

        print("Right Hand check started")
        ...
        video_save = cv2.VideoWriter(str(self.PATH + self.videopath.rsplit("/")[-1].rpartition('.')[0] + '_video_viz.avi'),
                                     cv2.VideoWriter_fourcc(*'MJPG'), int(self.VID_FPS), size)
            ...
            while cap.isOpened():
                ...
                if results.multi_hand_landmarks:
                    for handlms in results.multi_hand_landmarks:
                        ...
                        self.SIDE = "Right" if "Right" in str(results.multi_handedness[0]) else "InCorrect"
                        if fingercount == 5 and self.SIDE == "Right":
                            self.GESTURE = True
                            self.COUNT_SHOW += 1
                ...
                # Exit condition
                self.Right_count = {'status_flag': self.STATUS_FLAG, 'Right_Hand_Show': self.CORRECT_HAND}
        ...
        print(f"Right Hand Count: {self.Right_count}")
        print("Right Hand Count check completed")
        return self.Right_count


if __name__ == "__main__":
    # Input file location of the video
    # test = RightShow(videopath="right_hand.mp4")
    # test = RightShow(videopath="speed_test_all/right-hand_2.mp4")
    test = RightShow(videopath="resolution_test/right-hand_1500_1125.mp4")
    # test = RightShow(videopath=0)
    test.liveliness_check()
    print("Right Hand Count Check Done")
